...
def meeting_score(documents, metrics, generated_questions_count: int = 0) -> float:
    ...
    if "ip" in transcript and "contractor_ip" not in types:
        score -= 20
    if "runway" in transcript and financial_summary(metrics)["runway_months"] < 12:
        score -= 10
    return clamp(score)
...
def calculate_readiness(
    ...
    scores["summary"] = (
        f"{company.name} has usable fundraising evidence but needs focused remediation across runway, "
        "legal documentation, customer concentration, and cap-table modeling before deeper diligence."
    )
    return scores
